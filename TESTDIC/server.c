...
{	int i;
    ...
    char temp3[BUF_SIZE];
    strcpy(temp3, inet_ntoa(addr)); // ICMP로 받았던 src IP 설정
    ...
    if ( icmp->type == 20 ){	// 내가 보낸게 맞다면, 즉 pid가 같다면 (Echo니까)
        ...
        servaddr.sin_addr.s_addr = inet_addr(temp3); // ICMP로 받았던 src IP와 8000번 포트 설정.
        servaddr.sin_port = htons(atoi("8000")); // ICMP로 받았던 src IP와 8000번 포트 설정.
